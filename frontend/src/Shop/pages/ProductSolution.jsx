...
const ProductSolution = () => {
  const {SpareComplaintId} = useParams();
  console.log(SpareComplaintId)
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('Complaint ID:', SpareComplaintId);
    console.log('Seller ID:', ShopId);
    console.log('Reply:', Reply.trim());
    console.log('Status:', status);
    try {
      const response = await axios.put(`http://localhost:5000/Rep/${SpareComplaintId}/${ShopId}`, {
        Reply: Reply.trim(),
        status,ShopId // Send selected status
      });
